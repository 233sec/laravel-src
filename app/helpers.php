...
if (! function_exists('putFileUp')) {

    function putFileUp($fileinfo)
    {
        $mime_list = [
            ...
            //得到浏览器文件信息
            $_tmp_path = $fileinfo['tmp_name'];
            //得到后缀名
            if(array_key_exists($mime,$mime_list)){
                $extension = $mime_list[$mime];
            }else{
            ...
            try {
                ...
                $exploded = preg_split('/[\/\.$]/', password_hash(time().mt_rand(1000000,9999999),1));
                $filename = implode('', $exploded);
                $object = $filename.'.'.$extension;
                $ossClient->uploadFile($bucket, $object, $_tmp_path);

                $objectUrl = '//'.$bucket.'.'.$endpoint.'/'.$object;
